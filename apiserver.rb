...
class SignApiServer < Sinatra::Base
  ...
  sign = SignHandler.new(conf_json["DefaultMessage"])
  ...
  post '/message/new' do #write a new message to the sign
    ...
    data[:status] = sign.add(newid,data['message'],(data['color'].to_sym if data.has_key?('color')),(data['timer'] if data.has_key?('timer')))#adds message to the sign    
		data[:uuid] = newid
  ...
  put '/message/:uuid' do |id| #change message [uuid]
    ...
    data = JSON.parse request.body.read
    [:message,:color,:timer].each do |key|
    ...
    data[:status] = sign.add(id,data['message'],(data['color'].to_sym if data.has_key?('color')),(data['timer'] if data.has_key?('timer'))) ? "on" : "off"
    data[:uuid] = id
    data.to_json
  end
